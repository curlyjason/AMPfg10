...
echo $this->Html->div('addressSection', NULL);
	echo $this->Html->tag('h2', 'Billing Address', array('class' => 'toggle', 'id' => 'billingAddress'));
	echo $this->element('billing_address', array($billingAddress));
echo '</div>';//close billingAddressElement

echo $this->Html->div('addressSection', NULL);
	echo $this->Html->tag('h2', 'Order References', array('class' => 'toggle', 'id' => 'referencesAddress'));
	echo $this->element('references_address');
echo '</div>';//close orderReference

echo $this->Html->div('addressSection', NULL);
	echo $this->Html->tag('h2', 'Shipping Address', array('class' => 'toggle', 'id' => 'shippingAddress'));
	echo $this->element('shipping_address', array($myAddresses, $stateList, $countryList));
echo '</div>';//close shippingAddress

echo $this->Html->div('addressSection', NULL);
	echo $this->Html->tag('h2', 'Shipping Method', array('class' => 'toggle', 'id' => 'shippingMethod'));
